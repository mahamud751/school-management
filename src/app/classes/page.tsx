...
export default function ClassesPage() {
  ...
  const handleDeleteClass = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this class? This will also delete all associated sections and students."
      )
    )
      return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        const classItem = await res.json();

        if (currentClass) {
          // Update existing class
          setClasses(
            classes.map((c) => (c.id === classItem.id ? classItem : c))
          );
        } else {
          // Add new class
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        submitButtonText={currentClass ? "Update Class" : "Create Class"}
      >
        <div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
            placeholder="Enter level (e.g., Primary, Secondary)"
          />
        </div>
      </Modal>
    </motion.div>
